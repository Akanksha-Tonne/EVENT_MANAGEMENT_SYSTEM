...

               {
               	...
               	if($_SESSION['type'] != 'volunteer')
    			{
                  ...
                  $vname = $row[0];
                   ...
                   $fname = $row[5];
                   $db_connection = pg_connect("host=localhost dbname=event_management user=postgres password=database");
					$result = pg_query("select total_amt,amt_due from volunteer where vid='".$_SESSION['userid']."';");
					$row=pg_fetch_row($result);
...
if(count($_POST)>0) {
	...
	$result = pg_query("select total_amt,amt_spent,amt_due from volunteer where vid='".$_SESSION['userid']."';");
	$row=pg_fetch_row($result);
	if($row[2] < $amt)
	{
	...
	else
	{
		$amt_due = $row[2]-$amt;
		$amt_spent = $row[1] + $amt;
		$result = pg_query("UPDATE volunteer SET amt_spent=".$amt_spent." , amt_due= ".$amt_due." where vid='".$_SESSION['userid']."';");
