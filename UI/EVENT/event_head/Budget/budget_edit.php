<?php
	session_start();
 	if (!(isset($_SESSION['userid'])))
 	{
 		...
 		$ename = $row['ename'];
 		$db_connection = pg_connect("host=localhost dbname=event_management user=postgres password=database");
	$result = pg_query("select total_amt,amt_due from budget  where budget_id = (select budget_id_fk_event from event where ecoord_id='".$_SESSION['userid']."');");
...
if(count($_POST)>0) {
	...
	$result = pg_query("select total_amt,amt_paid,amt_due from budget  where budget_id = (select budget_id_fk_event from event where ecoord_id='".$_SESSION['userid']."');");
	$row=pg_fetch_row($result);
	if($row[2] < $amt)
	{
	...
	else
	{
		$amt_due = $row[2]-$amt;
		$amt_spent = $row[1] + $amt;
		$result = pg_query("UPDATE budget SET amt_paid=".$amt_spent." , amt_due= ".$amt_due." where budget_id= (select budget_id_fk_event from event where ecoord_id='".$_SESSION['userid']."');");
